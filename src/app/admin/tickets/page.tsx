...
export default function AdminTicketsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'pending':
        return <span className="bg-brand-accent/20 text-blue-800 flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border border-brand-accent/30"><Clock className="w-3 h-3 mr-1" /> Pending Response</span>;
      case 'resolved':
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white/70 backdrop-blur-xl border border-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden flex flex-col h-[calc(100vh-200px)] lg:col-span-1">
          <div className="p-4 border-b border-gray-100/50">
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl focus:ring-1 focus:ring-brand-accent outline-none text-sm bg-gray-50/50"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
                 <div className="p-10 flex justify-center"><div className="w-6 h-6 rounded-full border-t-2 border-r-2 border-brand-accent animate-spin"></div></div>
            ) : tickets.length > 0 ? (
                 ...
                 {tickets.map((t: any) => (
                   <div 
                    ...
                    className={twMerge(
                      ...
                      activeTicket?.id === t.id && "bg-brand-accent/10/50 border-brand-accent"
                    )}
                   >
                   ...
                   {replies.map((reply: any) => {
                      ...
                      return (
                        <div key={reply.id} className={twMerge("flex items-start gap-4", isAdmin && "flex-row-reverse")}>
                           <div className={twMerge("w-10 h-10 rounded-full flex items-center justify-center font-bold border", isAdmin ? "bg-brand-accent/20 text-brand-accent border-brand-accent/30" : "bg-gray-100 text-gray-600 border-gray-200")}>
                              {isAdmin ? 'A' : reply.sender_name.charAt(0)}
                           </div>
                           <div className={twMerge("flex-1 border rounded-2xl p-5 max-w-[85%]", isAdmin ? "bg-brand-accent/10 border-brand-accent/20 rounded-tr-none" : "bg-gray-50 border-gray-100 rounded-tl-none")}>
                              <div className="flex justify-between items-center mb-2">
                                 ...
                                 <span className={twMerge("text-xs", isAdmin ? "text-brand-accent" : "text-gray-500")}>{new Date(reply.created_at).toLocaleString()}</span>
                              </div>
                  ...
                  <form onSubmit={(e) => handleSendReply(e)} className="p-6 bg-white border-t border-gray-100/50">
                    <div className="relative">
                       <textarea 
                         ...
                         className="w-full resize-none p-4 pr-16 bg-gray-50 border border-gray-200 rounded-2xl focus:bg-white focus:ring-2 focus:ring-brand-accent focus:border-transparent outline-none text-sm transition-all shadow-inner"
                       />
                       <button 
                         type="submit"
                         disabled={!replyMessage.trim()}
                         className="absolute right-3 bottom-3 p-2 bg-brand-accent hover:bg-teal-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white rounded-xl shadow-md transition-colors"
                       >
